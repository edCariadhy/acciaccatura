...
- [ ] **Next/previous navigation within a scope:** a scope already reads in order in the sidebar (`bySequence` in [store.ts](../../packages/core/src/store.ts)) and every note is a clickable link (`vscode.open` in [treeView.ts](../../packages/extension/src/treeView.ts)) — so a walkthrough today is "open the scope, click down the list." There is no way to step to the next note in the sequence from wherever you currently are (editor focus, or having just read the previous one); you have to go back to the sidebar each time. A command bound to the currently-open scope, keyboard-reachable, would make the walkthrough a thing you drive from the code instead of the tree.
